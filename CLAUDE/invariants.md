...
- **The `other` documentation category must always sort last.** Why: a plain incrementing `sort_order` broke this the first time a category was added after it (seeded `ioc`, then a free-typed one both landed at or after `other`'s position) — the user caught it live. How to apply: `other` carries the sentinel `OTHER_CATEGORY_SORT_ORDER = 9999` (`server/src/db/seed.ts`); any code that assigns a new category's `sort_order` (`findOrCreateCategoryId` in `documentation.routes.ts`, or a future admin category-management UI) must compute its `MAX(sort_order)` excluding `key = 'other'`, never a plain unfiltered `MAX(sort_order) + 1`.
...
- **The app's own Azure Service Principal is never assigned `Contributor`/`Owner` — `Reader` at resource-group scope, plus narrowly-scoped additive roles only, if a later feature needs more.** Why: the identity used to *build* a customer's Azure environment and the identity this app authenticates as are different trust levels; broadening the app's own SP turns a credential leak into a subscription-wide compromise instead of a bounded read-only one. How to apply: environment registration instructions should tell the instructor to assign `Reader` on the resource group, never suggest `Contributor` as a shortcut. **Phase 2 (Bastion Connect + Key Vault credentials) is exactly the "later feature needs more" case**: additionally assign the custom `docs/azure/cyber-range-bastion-operator-role.json` role (Bastion Shareable Link actions only — `createShareableLinks`/`getShareableLinks`/`deleteShareableLinks`/`deleteShareableLinksByToken`, verified live against a real Bastion host on 2026-09-10) at the resource-group scope, plus the built-in `Key Vault Secrets Officer` role scoped to the environment's Key Vault resource specifically (not the resource group) so a compromised app credential still can't touch anything else in the vault's resource group. Never substitute Contributor for either. **Phase 3 (Run Script / Azure VM Run Command)** is the same pattern again: additionally assign `docs/azure/cyber-range-vm-run-command-role.json` (`Microsoft.Compute/virtualMachines/{read,runCommand/action,runCommands/read}` only) at the resource-group scope. Unlike Phase 2's Bastion role, this one has **not yet been empirically verified live** with a real narrow-permission Service Principal (see `BACKLOG.md`) — its action list is sourced from Microsoft's own VM Run Command IAM docs but the runtime code has only been exercised against validation paths (no live cloud environment was registered in this dev DB at the time), not a real `beginRunCommand` call end to end.
...
## Instructor Run Script (Phase 3)

### MUST

- **`script_executions` never stores raw script content or full command output** — only a SHA-256 `script_hash` plus a truncated, best-effort `output_excerpt`/`error_text` (capped at 4000 chars). Why: an instructor's script may embed credentials/tokens they're testing against, or a VM's output may echo them back — this is the same class of risk as a plaintext credential, and was an explicit approval condition before Phase 3 was built. How to apply: a **library** script's content is a deliberate exception (`scripts.content`, CONFIG) — the instructor chose to save it for reuse, so it's fine to persist; never add a `script_text`/`script_content` column to `script_executions` itself. `scriptExecution.service.ts#executeInBackground` is the only place Run Command output is handled, and it must keep truncating before writing to the DB or to `writeAudit`'s metadata.
- **A script's declared type must match the target VM's discovered OS before it's ever sent to Azure.** Why: `RunPowerShellScript` against a Linux VM (or vice versa) fails at the Azure layer anyway, but rejecting it in-app with a clear message (`reason: 'os_mismatch'`) is much better instructor UX than an opaque Azure error, and prevents ever silently coercing/re-typing an instructor's script. How to apply: `runScriptOnNode` derives the required type from `topology_nodes.metadata_json.osType` (`Windows`->`powershell`, `Linux`->`bash`) and compares before resolving credentials or calling Azure.
