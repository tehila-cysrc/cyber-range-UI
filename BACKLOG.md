...
- [ ] 2026-09-10 | manual | Phase 3's `ComputeManagementClient.beginRunCommand` call itself has not been exercised against a real Azure VM — the dev DB had no registered `cloud_environments` row at implementation time, so only the request-validation paths (not_runnable/invalid_request/os_mismatch/already_running, script CRUD) were tested live. Same caveat as the new `cyber-range-vm-run-command-role.json` RBAC role above — both need a real end-to-end run once an environment is registered again.
